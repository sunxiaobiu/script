...
public class CompareAPIPermission {
...
//        System.out.println(num);
    ...
    private static void getSDKHashLog(String logFolder, HashMap<String, String> sdkHashMap, String sdkVersion) throws IOException {
        File file = new File(logFolder + "/API_Body_" + sdkVersion + ".txt");
        ...
        for (int i = 0; i < fileContent.size(); i++) {
            ...
            String methodBody = currentLine.replace(api, "");

            if(StringUtils.isNotBlank(api)){
                currentAPI = api;
                sdkHashMap.put(api, methodBody);
            }else{
                String currentMethodBody = sdkHashMap.get(currentAPI);
                sdkHashMap.put(currentAPI, currentMethodBody + currentLine);
            }
        }
    }
